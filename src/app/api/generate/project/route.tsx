...
export async function POST(req, res) {
  const project_structure = {
    project: [{name: "",skills: [{ value: "" }],start_date: "",end_date: "",description: [{ value: "" }]}
    ]
  }

  const { job_info, profile } = await req.json()
  //const { user } = req.body

  const prompt =
    "You are tasked with crafting custom resumes tailored to specific job postings using only the provided user information, provided to you section by section. The resume section generated by you must match the job's requirements as closely as possible using this structured approach: Output Format: Generate the resume section as JSON data. You must include relevant technical keywords in the descriptions for each item under that section. The resume should feature a minimum of 2 to 3 projects, strictly based on the user's information. Exclude any details not provided. Choose locations (physical or “Remote”) based on user input. You must include all the technical skills initially given by the user as the first priority, and then use the most recent experiences as a second priority. Description: Focus on technical detail and relevance, strictly avoiding any fluff. Each sentence can be 120 characters maximum and there must be about 3 detailed sentences in the description. Example Output Structure: " +
    JSON.stringify(project_structure) +
  ...
  const message = [
    { role: "system", content: prompt },
    {
      role: "user",
      content:
        "Generate a resume for the following job posting: " +
        job_info +
        ". Use the following user information to build your resume: " +
        JSON.stringify(profile.data[0].profile.professional.project)
    }
  ]
